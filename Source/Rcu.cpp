...
=======
            ...
            int32_t AllocSlot(LirReg reg, int bytes) {
                ...
                // Minimum 8 bytes because MovArgStore/MovArgLoad always use
                // REX.W (64-bit) stores/loads regardless of type size.
...
=======
    ...
    // RISC-V codegen — implemented in RcuRiscv.cpp
    ...
    // Public API
    Rcu::Rcu(LirPackage package, std::string packageName, uint8_t arch)
...
<<<<<<< HEAD
=======
        if (arch_ == RcuArch::RISCV64) {
            for (const auto& mod : lir.modules) {
                result.push_back(
                    GenerateRiscV(mod, structDecls, interfaceNames, packageName));
            }
            return result;
        }
        for (const auto& mod : lir.modules) {
